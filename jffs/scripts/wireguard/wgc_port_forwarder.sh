...
#
# build_port_fwd_rules - flag: true if port forwarding rules need rebuild
# jumps_changed        - flag: true if forwarding jumps were modified
...
if ! [ -s "$port_fwd_rules" ]; then
    ...
    while IFS=: read -r ext_ports protos int_ip int_port; do
        ...
        case "$ext_ports" in
            *,*)
                ...
                if [ "$(count_mp_elems "$ext_ports")" -gt 15 ]; then
                    log -l warn "Too many ports for multiport (max 15): $ext_ports - skipping"
                    warnings=1
                    continue
                fi
            ...
            *-*)
                ...
                if [ -n "$int_port" ]; then
                    log -l warn "int_port is not supported for a port range - skipping"
                    warnings=1
                    continue
                fi
        ...
        case "$protos" in
            any) protos="tcp,udp" ;;
            tcp|udp|tcp,udp|udp,tcp) ;;  # allow both orders
            *)  log -l warn "Invalid proto spec '$protos' - skipping"
                warnings=1; continue ;;
        esac
