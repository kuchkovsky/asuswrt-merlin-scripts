...
#         Normalizes a protocol spec to one of following: "tcp", "udp", or "tcp,udp".
...
#
#   create_fw_chain [-q] <table> <chain> [-f]
...
#         Flush and delete a user-defined chain if it exists (no-op if missing).
#         -q suppresses informational logs (errors still logged).
#
...
validate_ports() {
    ...
    # Empty means "match all ports" (handled by caller) -> treat as valid.
    [ -z "$p" ] && return 0
    [ "$p" = "any" ] && return 0
    ...
    for tok in "$@"; do
        case "$tok" in
            ...
            *)
                ...
                [ "$tok" -ge 1 ] && [ "$tok" -le 65535 ] || return 1
                ;;
        esac
    done
