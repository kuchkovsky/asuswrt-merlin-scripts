...
#     * Run 'wfr' (helper alias) - recommended (reloads without reboot).
...
#     ips_<ports><t|u>_<set>[_<excl_set>]
#   Examples: ips_80,443t_blk   ips_22t_blk_exc1
...
# 5. Killswitch rules
# --------------------------------------------------------------------------------------------
# Temporarily block WAN ingress on specified port:proto pairs during the first
...
#
# Format:
#   port:proto - protocol can be "tcp", "udp", or "any" (matches both tcp and udp)
#
# Examples:
#   123:udp     # NTP
#   443:any     # HTTPS (both tcp and udp)
######################################################################################
KILLSWITCH_RULES='
'
...
#     * burst     -> allowance before throttling kicks in (hashlimit-burst)
...
#       - Defaults: minutes=5, log_count=1 -> about 1 log/IP every 5 minutes.
#
#   * per_port (dstport)
...
#       - Defaults: minutes=5 -> about 1 log every 5 minutes for all IPs.
#
# Logging:
...
#   The prefix format is:
#     dos_<ip|port>_<port><t|u>
#   Examples: dos_ip_443t   dos_port_123u
...
#
# Examples:
#   per_ip:123:udp:100:500        # per_ip on port 123 udp with defaults
#                                 # (minutes=5, log_count=1)
#
#   per_ip:80:tcp:200:400:10:5    # per_ip with explicit window and per-rule log cap
#                                 # (5 logs across 10 minutes)
#
#   per_port:123:udp:3333:5000    # per_port (log_count ignored; minutes defaults to 5)
##############################################################################################
DOS_RULES='
'
...
# 7. Timing logic
# --------------------------------------------------------------------------------------------
# Controls boot-time delay to avoid racing the firewall stack during early startup.
#
...
#
# This ensures ipset creation starts only after networking is fully initialized
# and any external storage (if used) is mounted.
#
# Note: If the killswitch flag (-k) is enabled, it is applied before the pause
...
KILLSWITCH_CHAIN='KILLSWITCH'

##############################################################################################
# 11. Status flags (shared between scripts)
# --------------------------------------------------------------------------------------------
# These temporary files are used to track ipset build state across runs and scripts:
#
# * IPSETS_CREATED_FLAG        - set after all ipsets are successfully created.
# * STORAGE_NOT_MOUNTED_FLAG   - set when external storage (e.g. /mnt/...) is missing.
# * DOWNLOAD_FAILED_FLAG       - set if any file (GeoLite2, custom lists) fails to download.
# * BUILD_FAILED_FLAG          - set if ipset build fails for any reason.
# * ALERT_EMAIL_SENT_FLAG      - prevents sending duplicate failure alert emails.
#
# Flags are cleared automatically when appropriate (e.g. on rebuild success).
##############################################################################################
IPSETS_CREATED_FLAG='/tmp/ipsets.created'
STORAGE_NOT_MOUNTED_FLAG='/tmp/ipsets_storage.not_mounted'
DOWNLOAD_FAILED_FLAG='/tmp/ipsets_download.failed'
BUILD_FAILED_FLAG='/tmp/ipsets_build.failed'
ALERT_EMAIL_SENT_FLAG='/tmp/ipsets_alert_email.sent'

##############################################################################################
# 12. Make all public configuration constants read-only
# --------------------------------------------------------------------------------------------
...
readonly \
    ...
    VSERVER_CHAIN FILTERING_CHAIN IPSET_CHAIN KILLSWITCH_CHAIN \
    IPSETS_CREATED_FLAG STORAGE_NOT_MOUNTED_FLAG \
