...
#   ipset_builder.sh -r        # reload firewall rules after run
...
# Tag for cron retry job
RETRY_CRON_JOB='retry_ipset_builder'
...
#   is_killswitch_requested    - true if killswitch=1 and KILLSWITCH_RULES is non-empty
...
#   compute_hash               - sha256sum of a file
#
# Network fetch:
...
create_retry_and_notify() {
    ...
    if ! cru l | grep -q "#${RETRY_CRON_JOB}#"; then
        ...
        [ "$reload" -eq 1 ] && retry_args="$retry_args -r"

        # Cron command, tagged so we can find / remove it later
        cron_cmd="*/10 * * * * $(get_script_path)${retry_args}"

        # Add cron job
        cru a "${RETRY_CRON_JOB}" "${cron_cmd}"

        log -l notice "Scheduled cron retry job '${RETRY_CRON_JOB}' (${cron_cmd})"
    fi
...
delete_retry_and_notify() {
    # If retry job exists
    if cru l | grep -q "#${RETRY_CRON_JOB}#"; then
        cru d "$RETRY_CRON_JOB" # delete it
        log -l notice "Removed cron retry job '$RETRY_CRON_JOB'"
...
compute_hash() { sha256sum "$1" | awk '{ print $1 }'; }
...
# -------------------------------------------------------------------------------------------------
if is_killswitch_requested; then
    # Create or flush helper chain
    if ! iptables -t raw -nL "$KILLSWITCH_CHAIN" >/dev/null 2>&1; then
        iptables -t raw -N "$KILLSWITCH_CHAIN"
        log "Created new chain $KILLSWITCH_CHAIN"
    else
        iptables -t raw -F "$KILLSWITCH_CHAIN"
        log "Flushed existing chain $KILLSWITCH_CHAIN"
    fi
    ...
    while IFS=: read -r port proto; do
        ...
        for p in $(printf '%s' "$proto" | tr ',' ' '); do
            iptables -t raw -A "$KILLSWITCH_CHAIN" \
                -p "$p" --dport "$port" -j DROP

            log "Added DROP rule -> $KILLSWITCH_CHAIN: port=$port proto=$p"
        done
    done <<EOF
$killswitch_rules
EOF
    ...
    # Insert jump once
    if ! iptables -t raw -C PREROUTING -i "$WAN_IF" \
           -j "$KILLSWITCH_CHAIN" 2>/dev/null; then
        iptables -t raw -I PREROUTING -i "$WAN_IF" \
            -j "$KILLSWITCH_CHAIN"
        log "Inserted jump: raw PREROUTING iface=$WAN_IF -> $KILLSWITCH_CHAIN"
    else
        log "Jump already exists: raw PREROUTING iface=$WAN_IF -> $KILLSWITCH_CHAIN"
    fi

    ports=$(printf '%s,' $killswitch_rules | sed 's/,$//')
...
if [ "$BOOT_WAIT_DELAY" -ne 0 ] && \
    ! awk -v min="$MIN_BOOT_TIME" '{ exit ($1 < min) }' /proc/uptime 2>/dev/null;
then
...
# -------------------------------------------------------------------------------------------------
# Let user know if firewall reload was requested
# -------------------------------------------------------------------------------------------------
if [ "$reload" -eq 1 ]; then
    log "Reload requested: forcing reload of firewall rules after build"
fi
...
if [ -z "$combo_ipsets" ]; then
    ...
    while IFS= read -r line; do
        ...
        for key in ${line//,/ }; do
            if ! ipset_exists "$key"; then
                log -l warn "Combo ipset '$set': member ipset '$key'" \
...
# If firewall reload requested or ipsets just created for the first time
if [ "$reload" -eq 1 ] || [ ! -f "$IPSETS_CREATED_FLAG" ]; then
    # Create the flag file to signal that ipsets are ready
    touch "$IPSETS_CREATED_FLAG"

    # Populate the filtering chain with ipset-based rules
    "$DIR/wan_firewall.sh"
fi
...
if is_killswitch_requested; then
    ...
    while iptables -t raw -D PREROUTING -i "$WAN_IF" \
        -j "$KILLSWITCH_CHAIN" 2>/dev/null; do :; done

    # Flush and delete the killswitch chain itself
    iptables -t raw -F "$KILLSWITCH_CHAIN"
    iptables -t raw -X "$KILLSWITCH_CHAIN"
