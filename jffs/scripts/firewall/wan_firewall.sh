...
build_dos_rules=0        # whether DoS protection rules should be built
build_ipset_rules=0      # whether ipset-based rules should be built
jump_rules_inserted=0    # whether chain jump rules have been inserted
...
# - Read the NAT ruleset from iptables-save.
# - In awk:
...
#       and a port was found:
#         * emit a single port directly (e.g. 443)
#         * expand a range like 81:84 into individual ports
# - Post-process:
...
get_ports() {
    ...
    awk -v P="$1" -v CH="$VSERVER_CHAIN" '
        $1 == "-A" && $2 == CH {
            ...
            if (port ~ /^[0-9]+:[0-9]+$/) {
                split(port, a, ":"); s = a[1] + 0; e = a[2] + 0
                for (p = s; p <= e; p++) print p
            } else if (port ~ /^[0-9]+$/) {
                print port + 0
            }
        }
...
strip_comments "$DOS_RULES" | sed -E 's/[[:blank:]]+//g' > "$dos_rules"
...
# Compute current DoS rules hash and load previous one if it exists
new_dos_hash="$(compute_hash "$dos_rules")"
old_dos_hash="$(cat "$DOS_RULES_HASH" 2>/dev/null || printf '%s' "$empty_rules_hash")"

# Compute current ipset rules hash and load previous one if it exists
new_ips_hash="$(compute_hash "$ipset_rules")"
old_ips_hash="$(cat "$IPSET_RULES_HASH" 2>/dev/null || printf '%s' "$empty_rules_hash")"

# Flag DoS rules for rebuild if:
...
# Flag ipset rules for rebuild if:
#   - Hash changed (list of rules was updated)
#   - Filtering chain is missing
#   - ipset chain is missing
if [ "$new_ips_hash" != "$old_ips_hash" ] \
    || ! fw_chain_exists raw "$FILTERING_CHAIN" \
...
###################################################################################################
need_hashlimit=0
[ -s "$dos_rules" ] && need_hashlimit=1
grep -q '^log:' "$ipset_rules" && need_hashlimit=1
...
elif [ "$build_dos_rules" -eq 0 ]; then
    log "DoS rules are applied and up-to-date"
else
    ...
    while IFS=: read -r mode port proto above burst minutes log_count; do
        ...
        case "$mode" in
            ...
            per_port)
                ...
                ;;
            *)
                log -l warn "Unknown DoS rule mode '$mode'; skipping rule"
        ...
        case "$port" in
            any|*[,-]*)
                log -l warn "Invalid 'port' spec '$port' -" \
                    "DoS rules require a single port (1-65535); skipping rule"
                dos_rules_warnings=1
                continue
                ;;
        esac
        ...
        if [ -z "$norm_proto" ]; then
            log -l warn "Invalid 'proto' spec '$proto' - expected 'tcp' or 'udp'; skipping rule"
            dos_rules_warnings=1
            continue
        fi
        case "$norm_proto" in
            ...
            *)
                # Disallow multi-proto ('tcp,udp' or 'any') for DoS rules
                log -l warn "Invalid 'proto' spec '$proto' -" \
                    "DoS rules require a single protocol ('tcp' or 'udp'); skipping rule"
                dos_rules_warnings=1
                continue
                ;;
        esac
        ...
        if [ -z "$burst" ]; then
            ...
            log -l warn "Invalid 'burst' value '$burst' - defaulting to 3"
            burst=3
            dos_rules_warnings=1
        fi
        ...
        case "$mode" in
            ...
            per_port)
                ...
                if [ -n "$minutes" ]; then
                    if ! is_pos_int "$minutes"; then
                        log -l warn "Invalid 'minutes' value '$minutes'" \
                            "for per_port rule - defaulting to 5"
                        minutes=5
                        dos_rules_warnings=1
                    fi
                else
...
###################################################################################################
ipsets_created_hash="$(cat "$IPSETS_CREATED_HASH" 2>/dev/null || printf '')"

if [ -s "$ipset_rules" ] \
    && [ "$build_ipset_rules" -eq 1 ] \
    && [ "$ipsets_created_hash" != "$new_ips_hash" ];
then
...
###################################################################################################
if [ "$build_ipset_rules" -eq 1 ]; then
...
# 9. Add ipset rules
###################################################################################################
if ! [ -s "$ipset_rules" ]; then
    log "No ipset rules are defined"
elif [ "$build_ipset_rules" -eq 0 ]; then
    log "ipset rules are applied and up-to-date"
else
    ...
    while IFS=: read -r mode ports protos keys excludes minutes ip_count; do
        ...
        case "$mode" in
            ...
            block|allow|pass)
                ...
                ;;
            *)
                log -l warn "Unknown ipset rule mode '$mode'; skipping rule"
                ipset_rules_warnings=1
                continue
                ;;
        esac
        ...
        if [ -n "$exclude_list" ]; then
            excl_set="$(derive_set_name "${exclude_list//,/_}")"

            if [ "$excl_set" = "any" ]; then
                log -l warn "Meta ipset '$excl_set' is not supported for exclusions; skipping rule"
                ipset_rules_warnings=1
                continue
            fi

            if ! ipset list "$excl_set" >/dev/null 2>&1; then
                log -l warn "Exclusion ipset '$excl_set' is missing; skipping rule"
                ipset_rules_warnings=1
                continue
            fi
        ...
        for proto in ${protos//,/ }; do
            case "$mode" in
                ...
                log)    # log only  -> log matching traffic (with rate limit), no accept or drop
                    ...
                    # - Result: ips_<ports>_<t|u>_<set>[_<excl_set>], e.g. "ips_80,443_t_blk".
                    log_name="ips_${port_list}_$(printf '%.1s' "$proto")_${set}${excl_set:+_$excl_set}"
...
# Save hash for the current run
printf '%s\n' "$new_ips_hash" > "$IPSET_RULES_HASH"

###################################################################################################
# 10. Finalize
###################################################################################################

if [ "$build_dos_rules" -eq 0 ] && [ "$build_ipset_rules" -eq 0 ] \
    && [ "$jump_rules_inserted" -eq 0 ];
then
    log "All firewall rules are already present"
elif [ "$dos_rules_warnings" -eq 0 ] && [ "$ipset_rules_warnings" -eq 0 ]; then
