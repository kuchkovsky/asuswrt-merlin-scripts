...
#          * If fstrim reports "Remote I/O error" or a generic "Input/output error":
#                -> The script retries using write_same_max_bytes (a safe hardware-provided limit,
#                   or a conservative fallback when unavailable).
...
# Hardcoded fallback for discard_max_bytes when
# write_same_max_bytes is unusable (missing, zero, or non-numeric)
FALLBACK_DISCARD_MAX_BYTES=33553920  # ~32 MiB
...
ensure_unmap_for_disk() {
    ...
    for pm in /sys/block/"$disk"/device/scsi_disk/*/provisioning_mode; do
        ...
        if [ "$cur" != "unmap" ]; then
            if echo -n unmap > "$pm" 2>/dev/null; then
...
#         - Mark disk as unsupported: nvram key -> 0 (permanently skipped).
#     * "Remote I/O error" or generic "Input/output error":
#         - If no override was used yet:
#             + Read write_same_max_bytes; if missing/invalid/zero, fall back to a
#               conservative fixed value (~32 MiB).
#             + Try that value as discard_max_bytes and rerun fstrim.
#             + On success: store the working value in nvram and reuse it next time.
#         - If an override was already used, or the fallback still fails:
#             + Mark disk as unsupported: nvram key -> 0 (permanently skipped).
# -------------------------------------------------------------------------------------------------
run_fstrim_with_fallback() {
    ...
    local output clean_output ws_bytes_raw ws_bytes bytes

    # If we already have a cached working value, apply it first
    if [ -n "$nv_val" ]; then
        used_override=1
        if echo -n "$nv_val" > "/sys/block/$disk/queue/discard_max_bytes" 2>/dev/null; then
    ...
    # Strip common fstrim prefix to keep logs cleaner
    clean_output="$output"
    clean_output="${clean_output#fstrim: FITRIM: }"

    # Log error
    log -l err "fstrim failed on $mp: $clean_output"
    ...
    case "$output" in
        *"Operation not supported"*)
            ...
            return 1
            ;;
        *"Remote I/O error"*|*"Input/output error"*)
            ...
            if [ "$used_override" -eq 0 ]; then
                ...
                ws_bytes="$ws_bytes_raw"

                case "$ws_bytes_raw" in
                    ''|*[!0-9]*|0)
                        # write_same_max_bytes is missing, invalid
                        # or zero -> use hardcoded fallback
                        ws_bytes="$FALLBACK_DISCARD_MAX_BYTES"
                        log -l warn "write_same_max_bytes='${ws_bytes_raw:-empty}'" \
                            "for /dev/$disk; trying fallback discard_max_bytes=$ws_bytes"
                        ;;
                esac

                if echo -n "$ws_bytes" > "/sys/block/$disk/queue/discard_max_bytes" 2>/dev/null
                then
                    ...
                    if output=$(fstrim -v "$mp" 2>&1); then
                        clean_output="$output"
                        clean_output="${clean_output#fstrim: FITRIM: }"
                        log "fstrim succeeded with fallback discard_max_bytes=$ws_bytes:" \
                            "$clean_output"
                    ...
                    log -l err "fstrim still failed on $mp with fallback" \
                        "discard_max_bytes=$ws_bytes: $clean_output"
                else
...
while read -r dev m fstype rest; do
    case "$m" in
        /tmp/mnt/*)
            ...
            if run_fstrim_with_fallback "$disk_name" "$m" "$context" "$nv_key" "$nv_val"
            then
                trimmed=1
            else
                trim_failed=1
