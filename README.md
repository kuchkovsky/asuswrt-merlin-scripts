  ...
  * **Flexible ipset rules**  
    - Defines filtering behavior using a clear rule format:  
      **`mode:ports:protos:keys[,keys...]:excludes[,excludes...][:minutes][:ip_count]`**  
      ...
      - `keys` → one or more ipsets or combo sets (`list:set`)  
      - `excludes` → optional sets to exempt from a match  
    ...
    - Features:  
      ...
      - **Combo sets** (`list:set`) combine multiple sets into a single match key for simpler, faster rules.  
...
* [**`wf_ib_shared.sh`**](jffs/scripts/firewall/wf_ib_shared.sh) - internal shared library for both
  `wan_firewall.sh` and `ipset_builder.sh`.

* [**`services-start`**](jffs/scripts/services-start) - Asuswrt-Merlin hook invoked on boot completion.
  It triggers the initial ipset build and schedules periodic refresh via cron (daily for country-based sets
...
* [**`profile.add`**](jffs/configs/profile.add) - Asuswrt-Merlin custom shell profile config. 
  ...
  by simply typing `ipb` (ipset builder), without rebooting.
...
## 2. WireGuard client port forwarder
> **Prerequisites:**
...
* [**`profile.add`**](jffs/configs/profile.add) - Asuswrt-Merlin custom shell profile config. 
  Defines a helper alias so you can edit `config.sh` and reapply port forwarding changes instantly
  by simply typing `wpf` (WGC port forwarding), without rebooting.
...
**Purpose** - provide common building blocks (structured logging, per-script locking, temp files/dirs with
  auto-cleanup, SHA-256 hashing, comment stripping/normalization, port/protocol validation & normalization,
  idempotent `iptables` helpers, selective WAN-blocking utilities, and a lightweight email notifier)
  so feature scripts stay short, focused, and easy to maintain.
...
* [**`firewall.sh`**](jffs/scripts/utils/firewall.sh) **library**
   ...
   | `create_fw_chain [-q] <table> <chain> [-f]`                                             | Ensures a user-defined chain exists; with `-f` flushes it if already present. `-q` suppresses info logs.                                                                                                                                                                       | Initialize or reset per-script chains (e.g., `RAW_FILTERING`, `WGC1_RULES`).                                                                                        |
